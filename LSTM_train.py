...
from sklearn.preprocessing import MinMaxScaler  #for standardizing
...
#loading data function  (input to this function is data_filepath)
...
#function split data into training and test
    #input to function is test_size as proportion and input dataframe
def split_data(input_test_size, df):
    """
...
#only intrested in Date and Close (price of stock)
    train = df[:train_num][['Date', 'Close']]
    test = df[train_num:][['Date', 'Close']]

    return train, test

#to combine a number of dates in a group, e.g. 60
...
#function model setup, compile, fit
def fit_lstm(X_train, y_train, lstm_units = 50, optimizer = 'adam', epochs = 1,
    ...
    INITIAL parameters chosen for LSTM
    lstm_units = 50
    optimizer = 'adam'
    epochs = 1
    batch_size = 1
...
def model_performance(test, closing_price):
    ...
    """
    #to write data to this dataframe
...
def train_pred_eval_model(X_train, y_train, df, scaler, test,
    ...
    #predict closing price for last 60 values
    closing_price_tuned = get_pred_closing_price(df, scaler, model_tuned)
    #evaluate model performance by calculating rmse and mape errors
    rmse_lstm, mape_lstm = model_performance(test, closing_price_tuned)
    return rmse_lstm, mape_lstm

#saving model with a pickle
...
#for running from command prompt
        
        # 1. read data from file and split to train test, standardize
        # 2. fit the lstm model on data, predict and evaluate
        # 3. save model and scalar file
def main():

    #sys.argv is a list in Python which contains the command-line arguments passed to script
    #with len(sys.argv) function I can count the number of arguments
    if len(sys.argv) == 7:
